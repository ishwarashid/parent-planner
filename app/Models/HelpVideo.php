...
class HelpVideo extends Model
{
    ...
    public function getVideoUrlAttribute($value)
    {
        ...
        // Check if this is a help video path - if so, it should be public and use CDN
        $isHelpVideoPath = strpos($value, 'help-videos/') === 0;
        
        if ($isHelpVideoPath) {
            // For help videos, directly use the do_spaces_url helper which handles CDN
    ...
    public function getThumbnailUrlAttribute($value)
    {
        ...
        // Check if this is a help video thumbnail path - if so, it should be public and use CDN
        $isHelpThumbnailPath = strpos($value, 'help-video-thumbnails/') === 0;
        
        if ($isHelpThumbnailPath) {
            // For help video thumbnails, directly use the do_spaces_url helper which handles CDN
            return do_spaces_url($value);
        }
